        ...
        <article class="text-dark flex flex-col gap-4">
            ...
            @if ($book->available_copies)
                <a href="/borrow/{{ $book->id }}"
                    class="bg-dark text-light mt-5 block text-center p-2 rounded hover:text-light/90 hover:bg-dark/90 transition-all w-1/2 ">
                    Borrow
                </a>
            @else
                <button
                    class="bg-dark/50 mt-5 cursor-not-allowed disabled:true block text-center p-2 rounded transition-all w-1/2">Checked
                    out
                </button>
            @endif
