<x-admin-layout>
    ...
    <div x-data="{
        ...
        showImportMembersModal: false, // NEW: Alpine.js variable for import modal
        ...
        @endif
        // Open import modal if there are validation errors from import
        @if ($errors->importingMembers->isNotEmpty())
            showImportMembersModal = true;
        @endif
    ">
        <div class="py-12">
        ...
        <div x-show="showImportMembersModal" class="fixed inset-0 z-50 overflow-y-auto"
            aria-labelledby="import-modal-title" role="dialog" aria-modal="true" x-cloak>
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                <!-- Background overlay -->
                <div x-show="showImportMembersModal" x-transition:enter="ease-out duration-300"
                    x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                    x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100"
                    x-transition:leave-end="opacity-0"
                    class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true"></div>
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500 mb-4">Upload a CSV file containing member details.
                                        The CSV should have columns: `full_name`, `email`, `reg_number`, `fee_balance`.
                                    </p>
                                    <form method="POST" action="{{ route('admin.members.import') }}"
                                        ...
                                        <div class="mb-4">
                                            <label for="import_file"
                                                class="block text-sm font-medium text-gray-700">Choose CSV File</label>
                                            <input type="file" id="import_file" name="import_file" accept=".csv"
                                                class="mt-1 block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:outline-none">
                                            @error('import_file', 'importingMembers')
                                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                            @enderror
                                        </div>
                                        <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                                            ...
                                            <button @click="showImportMembersModal = false" type="button"
                                                class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
                                                Cancel
                                            </button>
                                        </div>
